...
def main(args, save_dir, model_path, tst_path=None, max_len=0, skip_timesteps=0, tst_prefix=None, dataset='BEAT', 
    ...
    if tst_path is not None:
        ...
        # Obtener el nombre del archivo sin la extensión
            filename = os.path.basename(wav_file).replace('_ps_3_spk_12.wav', '') #vozes transformados
            #filename = os.path.basename(wav_file).replace('.wav', '') #vozes originales

        # Construir el camino del archivo .tsv correspondiente
            txt_file = os.path.join(txt_path, filename + '.tsv')
            ...
            # Procesar el archivo de texto
                tsv = load_tsv(txt_file, word2vector, clip_len)
                textaudio = np.concatenate((wav, tsv), axis=-1)
                textaudio = torch.FloatTensor(textaudio)
                textaudio = textaudio.to(mydevice)

            # Configurar el estilo del hablante
                speaker = np.zeros([17])
                speaker[10] = 1  # random choice will be great
                filename = os.path.basename(wav_file).replace('.wav', '') #vozes transformados

            # Ejecutar la inferencia
                inference(args, save_dir, filename, textaudio, sample_fn, model, n_frames=max_len, smoothing=True,
                      skip_timesteps=skip_timesteps, style=speaker, seed=123456, dataset=dataset)
            else:
                print(f"Archivo de texto {txt_file} no encontrado para {wav_file}")
        #######################################
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_path', type=str, default='./TWH_mymodel4_512_v0/model001200000.pt') #./model000450000.pt
    ...
    parser.add_argument('--word2vector_path', type=str, default='../process/crawl-300d-2M.vec')
    ...
    # Extraer el nombre de la carpeta a partir del wav_path y eliminar el prefijo "wav_"
    wav_path1 = args.wav_path
    wav_folder_name = os.path.basename(os.path.normpath(wav_path1))

    # Si el nombre de la carpeta comienza con "wav_", eliminar ese prefijo
    if wav_folder_name.startswith("wav_"):
        wav_folder_name = wav_folder_name[4:]
    
    config.save_dir = "./" + model_root + '/' + 'sample_' + model_spicific + '/' + 'bvh_' + wav_folder_name + '/' #vozes com ruidos
    # config.save_dir = "./" + model_root + '/' + 'sample_dir_' + model_spicific + '/' + 'bvh_sp11m_ps-2_sp20m_lowest' + '/' #vozes transformadas
    #config.save_dir = "./" + model_root + '/' + 'sample_dir_' + model_spicific + '/' + 'bvh_sp11' + '/' #vozes originales
